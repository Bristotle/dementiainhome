...
export type PublishedPageWithContext = GeneratedPageRecord & {
  city: { slug: string; name: string; state: string; state_abbrev: string; hourly_rate_low: number; hourly_rate_high: number }
...
export async function getPublishedPage(citySlug: string, templateSlug: string): Promise<PublishedPageWithContext | null> {
  const { data, error } = await supabase
    .from("pages")
    .select("id, title, meta_description, content_json, published_at, cities!inner(slug, name, state, state_abbrev, hourly_rate_low, hourly_rate_high), master_templates!inner(topic_type, intent)")
  ...
  return {
    ...
    city: data.cities as unknown as PublishedPageWithContext["city"],
