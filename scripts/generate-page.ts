...
- NEVER TURN A SOURCE'S SILENCE INTO A CLAIM. If a page does not mention something, that establishes nothing about whether it exists - do not write that a state "does not offer", "does not publish" or "has no" something merely because its page is quiet about it. Say what the source does describe, and where a reader needs more, tell them who to ask.
- HOW TO CITE THE MEDICAID PROGRAM DATA: each field in the Medicaid record above carries its own provenance, and you must follow it. Where a field states a figure plainly - an asset limit, an income cap, a look-back period - that figure came from the program page cited in the record, and you should attach that citation to the sentence containing it. Where a field instead says the figure is "not published on the cited page", that means nobody has a citable source for it: do not state a number, do not approximate one, and do not attach any citation - say the limit is set by that state's Medicaid rules and tell the reader to confirm the current figure with the office that administers the programme. Never attach a federal source (CDC, NIA, Medicare, VA) to a state Medicaid figure; those pages say nothing about any state's limits.
...
- NO EN-DASHES OR EM-DASHES: do not use the en-dash (\u2013) or em-dash (\u2014) anywhere in the content, and do not write them as &ndash; or &mdash;. Use a regular hyphen (-), a comma, or restructure the sentence.
