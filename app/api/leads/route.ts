...
async function sendLeadNotification(lead: {
  ...
  city?: string; state?: string; message?: string; page_type?: string; source_page?: string
}) {
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [notifyEmail],
        reply_to: lead.email,
        subject: `New lead: ${lead.first_name} ${lead.last_name} (${lead.city || "unknown city"})`,
        html: `
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { first_name, last_name, email, phone, city, state, message, page_type, source_page } = body
    if (!first_name || !last_name || !email || !phone) {
      return NextResponse.json({ error: "Please fill in all required fields." }, { status: 400 })
    }
    const { error } = await supabase.from("leads").insert([{ first_name, last_name, email, phone, city, state, message, page_type, source_page }])
    ...
    await sendLeadNotification({ first_name, last_name, email, phone, city, state, message, page_type, source_page })
