    ...
    <main className="min-h-screen bg-warm-white">
      ...
      <section className="bg-slate-50 border-b border-slate-200 py-20 bg-soft-wash">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 mb-6">The dementia care specialist families trust</h1>
          <p className="text-lg text-slate-600 leading-relaxed max-w-2xl mx-auto">We are building the most trusted in-home dementia care matching service in the United States — one family, one caregiver, one city at a time.</p>
        </div>
      </section>
      <section className="max-w-5xl mx-auto px-6 py-16 bg-glow-center">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-16">
          <div>
            ...
            <p className="text-slate-900 font-bold text-lg">"Dignity at home. Peace of mind for family."</p>
          </div>
          <div className="bg-teal-50 rounded-3xl p-8 border border-teal-200">
            <div className="grid grid-cols-2 gap-4">
              {[["346","US cities in growth plan"],["7.4M","Americans with Alzheimer's"],["$0","Cost to get matched"],["72hrs","Our matching guarantee"]].map(([val,label]) => (
                <div key={label} className="bg-white rounded-2xl p-4 border border-teal-200 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12">
          {[
            ...
            <div key={title} className="bg-slate-50 rounded-2xl p-6 border border-slate-200 hover:border-teal-300 transition-colors">
              <h3 className="font-bold text-slate-900 mb-2">{title}</h3>
              <p className="text-slate-600 text-sm leading-relaxed">{desc}</p>
            </div>
          ))}
        </div>
        <div className="bg-teal-600 rounded-3xl p-8 text-white text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              <a href="/#get-matched" className="px-8 py-3 rounded-xl bg-white text-teal-600 font-semibold hover:bg-teal-50 transition-colors">Get Free Caregiver Profiles</a>
              <a href="/contact" className="px-8 py-3 rounded-xl bg-teal-700 text-white font-semibold hover:bg-teal-800 transition-colors border border-teal-500">Contact Us</a>
            </div>
          </div>
        </div>
      </section>
